...
> ⚠️ 2026-07-23：本文多处拿 `claude_code`/mcp_bridge 当"路径外型"范例，但该
> backend 已删除。范例讲的**设计模式**（路径外 vs 路径内）仍有效，但现网的
> 路径外实现是 `claude_channel`（daemon 直接读写 inbox/outbox，无 mcp_bridge）。
> 读到 claude_code/mcp_bridge/chats-loop 时理解成"某个路径外 backend 的历史写法"。

后端是"把用户消息变成回复"的那一端：Claude Code（channel，当前）、Hermes ACP、
