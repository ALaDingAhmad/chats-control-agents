...
- **在线 = `bridge_pid` 活 && marker 新鲜**。bridge 进程活着只说明 MCP server
  ...
  **mtime 是否在 TTL（`paths.LOOP_MARKER_TTL_SECS`，180s）内**。marker
  是租约不是开关：mcp_bridge 在 `wait_for_message` 阻塞期间每 ~5s touch
  一次、`send_chat_response` 时也 touch；循环停了（哪怕进程还活着、哪怕
  ...
  已知取舍：收到消息后埋头处理超过 TTL 才回复的那段时间会短暂显示离线——
  状态显示保守，不影响消息投递。
...
是否仍在收件"要看它的 **mtime 新鲜度**，不是存在性——mcp_bridge 在
`wait_for_message` 等待期间每 ~5s touch、`send_chat_response` 时也 touch，
构成心跳租约（TTL 见 `paths.LOOP_MARKER_TTL_SECS`）。文件不出现 = skill
没起来（trust 弹窗卡住、READY_MARKERS 不匹配、trigger 没进 TUI…）；
文件在但 mtime 陈旧 = 循环已停或进程被硬杀后的残留。
