...
description: |-
  ...
  invoked, Claude enters a never-ending loop using the `web-chat` MCP server:
  ...
  (every phrase contains the literal token `web-relay` — anything that does
  not contain that token must NOT trigger this skill):

  English: "start web-relay", "begin web-relay", "open web-relay",
  "enter web-relay", "launch web-relay", "activate web-relay", "run web-relay",
  "/web-relay", "/webrelay".

  Chinese: "启动 web-relay", "进入 web-relay", "调用 web-relay",
  "开启 web-relay", "开始 web-relay", "运行 web-relay", "激活 web-relay".

  Do NOT trigger on: "web-chat", "启动 web-chat" (that is the MCP server
  name, not this skill — they are different things), "开始", "start",
  "go", or any phrase missing the literal `web-relay` token.
---
...
(They are provided by an MCP server named `web-chat` — note: the server
name `web-chat` is unrelated to this skill's name `web-relay`. Do not get
confused by the name collision.)

- `mcp__web-chat__wait_for_message()` — blocks until the web user sends a
...
- `mcp__web-chat__send_chat_response(reply)` — sends your full reply back
  to the web user.
...
1. **Compute the alias** for this relay session — never reuse a generic name
   ...
   - Examples: `agent-bridge-0605-1130`, `ALIENWARE-0605-0903`,
     `项目-X-0605-1500`.
2. Call `mcp__web-chat__relay_init` with that alias as the `alias` argument.
   If it returns a string starting with `ERROR`, fix the alias and retry once.
3. Print exactly one terminal line: `web-relay loop active alias=<alias>`
4. Immediately call `mcp__web-chat__wait_for_message` (no arguments needed)
   and proceed to the loop below.

## The loop — follow exactly, do not deviate

Repeat forever until the terminal user types "stop web-relay" or
"停止 web-relay":

1. Call `mcp__web-chat__wait_for_message` (no arguments needed).
...
3. **Otherwise** (you got real message text from the web user):
   ...
   b. Call `mcp__web-chat__send_chat_response` with your full reply as
...
- **Never** stop the loop on your own. Only stop when the terminal user
  explicitly types "stop web-relay" / "停止 web-relay".
