"""
PreToolUse hook for web-relay mode.

Fires immediately BEFORE Claude executes mcp__web-chat__send_chat_response.
...
`mcp__web-chat__send_chat_response` — that means this hook ONLY fires for
...
Stdin (Claude Code → hook) JSON:
  {
    ...
    "tool_name": "mcp__web-chat__send_chat_response",
...
Log: ~/.claude/hooks/web_relay_pretool_hook.log
"""
import json
import logging
import sys
...
LOG_PATH = Path.home() / ".claude" / "hooks" / "web_relay_pretool_hook.log"
...
def _harvest_turn_narration(
    ...
    # mcp__web-chat__wait_for_message — that result carries the web user's
    ...
    for i in range(target_idx, -1, -1):
        ...
        for c in content:
            if (
                ...
                and c.get("name") == "mcp__web-chat__wait_for_message"
            ):
                wait_id = c.get("id")
                break
