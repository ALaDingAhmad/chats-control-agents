...
- **手开的 claude.exe 不会污染会话**：commit `167f1d9` 之后，没设 `CHATS_LOOP_ALIAS` env 的 mcp_bridge.py 会用 `<basename(cwd)>-<MMDD-HHMM>` 拿独立 alias，不抢 daemon-spawned session 的 inbox。所以 `ps` 看到的额外几个 mcp_bridge.py 进程是无害的，不要顺手杀。**但 mcp_bridge 绝不能写 `_current.txt`**——2026-07-16 一版半成品在 bridge 启动时抢路由指针，导致随手开个 claude 窗口就劫持全部微信入站、消息落进没人消费的 inbox 全网静默。"成为当前会话"只能靠用户显式选择。相关契约见 `docs/入站路由.md` "终端 chats-loop 会话（bridge-owned）"。
