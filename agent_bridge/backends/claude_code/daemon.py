...
finish loading, then auto-types "启动 web-relay\\r" so the web-relay skill
...
What this proves (or disproves):
  ...
  - Does the web-relay skill actually trigger when fed via PTY?
...
# ("skill web-relay") because the bare phrase "启动 web-relay" sometimes makes
# Claude treat web-relay as a service name and try to investigate how to
...
TRIGGER_COMMAND = "调用 web-relay skill 立即进入消息循环"
...
def main() -> int:
    ...
    # interactive prompts. Crucially: set WEB_RELAY_ALIAS so the child claude
    # → mcp_bridge subprocess knows which session it serves.
    spawn_env = {**os.environ, "WEB_RELAY_ALIAS": ALIAS}
    ...
    # output. We're looking for the skill's "web-relay loop active" line.
    ...
    while time.time() - confirm_start < POST_TRIGGER_SETTLE * 3:
        ...
        pty_log.flush()
        confirm_buffer += text
        if "web-relay loop active" in confirm_buffer or "loop active" in confirm_buffer:
            activated = True
            log.info("skill activated")
            print("[daemon] OK: web-relay loop active")
            break

    if not activated:
        log.warning("did not see 'web-relay loop active' within %ds; continuing anyway "
    ...
    while proc.isalive():
        try:
            ...
            if chunk:
                ...
                if rate_limited and (
                    "web-relay loop active" in pty_buffer
